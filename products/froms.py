...
class ProductForm(forms.ModelForm):
    ...
    # 상품 폼을 생성할 때! 중요한건 사용자다 -> 누가 이 상품을 등록하는지가 중요하다.
    # 사용자를 뽑아서 해ㅅ당 상품이랑 연결시켜주기 위함.
    ...
    # save 메서드는 폼에서 입력한 데이터를 db에 적재하는 놈
    def save(self, commit=True):
        # 부모 클래스의 save 매서드를 호출하되, commit=False 할것이다 -> 해당삼품은 들고오되 db에는 해당 product를 저장하지 않겠다.
        ...
        # 해시태그 처리
        # 입력받은 hashtag_str 문자열을 쉼표나 공백으로 구부하기(제한)
        # 각 해시태그를 db에 저장하거나, 이미 존재하면 가져올것
