...
class Product(models.Model):
    ...
    # 사용자가 상품에 '좋아요'를 누를 수 있도록(그런 관계)
    # user like(좋아요는) 어떤 관계 -> ManyToManyField 다대다 관계를 설정 가능(User 모델이랑)
    # 상품에 좋아요가 없을 수도 있다 blank=True
    # 사용자가 좋아요를 누른 상품을 user.liked_products 참조 가능하다. -> 상품들 리스트가 나옵니다
